...
\begin{abstract}
  ...
  \sout{In the creation we've used the Flask-codebase as our starting
    point and huge (isolated) parts of the code was left unchanged.}
  ...
  event-handling. \fixme{Any other major additions?}
...
\fixme{Check all cites. Should there be a space between the word and cite or
  not.}
\fixme{Check all refs. Should be converted to vref to utilize the
  varioref package (Where appropriate!!)}
...
often\fixme{citations} defined as \textit{programmable systems that
  senses and regulates some task by the use of sensors and actuators},
which includes almost any computer-system. Selecting the
...
concentrating on the Arduino isn't a very strict limitation. This has
the obvious impact, that our framework should be general enough to
support almost any Arduino-connectable external device. We do this by
providing a way of writing drivers or small frameworks for arbitrary
devices.
...
often called signals).\fixme{citation}
...
In the following chapter (\Fref[plain]{chap:Flask}) we will go
...
In \fref{chap:implementation} we will talk about the
technically modification we have made to transform Flask to Fladuino.

In \fref{chap:evaluation} we will present an evaluation of
the complete Fladuino system, judging its benefits over traditional
methods of programming control programs, as well as covering
...
before we can go on to the next action. \fixme{eventually add
  something about the need for this} This point is actually easier to
explain when talking about Fladuino. The good example here is that we
might want two different \textit{atomic subgraphs} to execute
...
library, but you won't get the Arrow syntax\cite{PatersonRA:notation}.
...
two type classes: \textit{Reify} and \textit{LiftN}.  Instances of the
\textit{Reify} type classes are Haskell types that has node-level
equivalents. An instance has to provide a function \texttt{reify} that
given a Haskell value returns a representation of its node-level type.

\textit{LiftN} is a multi parameter type class\footnote{This requires
  the GHC language extension MultiParamTypeClasses}. An instance
\texttt{LiftN eta Integer} represents that the Haskell value of type
can be lifted to a node-level \textit{Integer}.\\
