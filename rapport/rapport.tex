...
represents how data from sensors etc. are processed and sent to other
nodes in the network. When a Flask program is "`run"', it generates a
program in another more low-level language for sensor networks. Our
framework, Fladuino, follows exactly the same concepts as Flask,
although its generates C-code written specifically for our platform:
Arduino, which we will discuss in the next section, \ref{sec:hardware
  platform}.
...
\textit{any programmable electronic system with sensors and/or
  actuators}. This has the obvious impact, that our framework should
be general. In the sense that we should support 

\fixme{I find the following section relevant, but I don't know where
...
a framework for developing these kinds of systems they finally bought
some and let us use them. Even better reasons is that they are cheap
...
comes with a relatively high-level C-interface, sparing us from much
...
expressible in NesC, and NesC is also heavily used in the
implementation of Flask. The other language, Red, is described below.
