...
We expect that readers of this report has a small background knowledge
of electronics.

Additionally, we expect the reader to have casual familiarity with the
concepts of Quasiquotation \cite{quasiquote07}, phantom types, and to
know the difference between meta-languages versus object-languages.
...
In this section we first show Fladuino-code for a line following
robot. The body of the implementation is shown in the following
figure (\ref{fig:linefollow}).
...
It expresses a simple line-following strategy. Whenever the CPU is
ready (\texttt{idle}), read the value of the reflectance sensors
(\texttt{valueOf ReflectanceSensors}), uses these values to calculate
the speed of the two motors (\texttt{smap velocity}, the
\texttt{velocity} function is explained below), and update the
velocities of the motors with these values (\verb|set_motors Motors|).
...
value between $0$ and $4000$ indicating the position of the line. When
the line is to the right of the robot, the value is below $1000$ and
we turn left. If the line is centered on the line, the value is
somewhere between $1000$ and $3000$ and we continue straight a
head. Values above $3000$ indicates that the line is to the left of
the robot and we turn right. The implementation of this is shown in
\fref{fig:velocity-function}

\begin{figure}
\begin{verbatim}
velocity :: N (Integer -> (Integer, Integer))
velocity = liftN [$decls|f linepos = 
                    if linepos < 1000 then (0, $int:speed)
                    else if linepos > 3000 then ($int:speed, $int:speed)
                    else ($int:speed, 0)|]
\end{verbatim}  
  \caption{\texttt{velocity}-function}
