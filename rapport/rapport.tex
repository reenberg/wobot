...
Bluetooth communication to the Atmega chip may break if. The WT11 chip is
configured wrong it may not accept connections or avrdude may not be able to
communicate with the boatloader.
...
which connection the data sequence should be sent to or is received from. Table
\ref{tab:multiplexing_protocol} illustrates the multiplexing frame format.
...
When multiplexing mode has been enabled\footnote{To enable multiplexing mode
  send the iWRAP command \texttt{SET CONTROL MUX 1}}, it can be disable by the
following sequence \texttt{BF FF 00 11 53 45 54 20 43 4f 4e 54 52 4f 4c 20 4d 55
  58 20 30 00} which breaks down to the following
...
\begin{center}
  \begin{tabular}{|l|p{300pt}|}
    ...
    \textsf{DATA} & Is \texttt{53 45 54 20 43 4F 4E 54 52 4F 4C 20 4D 55 58 20 30} which is the hex
    representation of \texttt{SET CONTROL MUX 0} \\ \hline
    \textsf{nLINK} & is \texttt{0x00} as \texttt{0xFF xor 0xFF = 0x00} \\ \hline
  \end{tabular}
\end{center}
 
iWRAP commands should not end with ``$\backslash$r'' or
``$\backslash$r$\backslash$n'' when encoded to their hex representation and used
in a multiplexing frame.
...
\begin{quote}
  In MUX mode the processor of the module is highly utilized and on the edge of
