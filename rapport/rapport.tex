...
\begin{nonfloatingfigure}
  \includegraphics{flask-design.png}
...
generated. A program is a description of the flow of data in the
specific sensor node. A node has some sensor inputs and can output the
values in different ways e.g. send results to other devices. So you
specify an acyclic graph connecting its sensors (which generates
values on request) with its outputs. Flask provides some general
\textit{signal functions} that combined with \textit{node-level code}
(code that is actually getting to run on the sensor-nodes) written in
one of the object-languages, can be used to specify this
\textit{dataflow graph}.
