...
\chapter{Flask}
\label{chapter:Flask}
\fixme{mangler noget om opdelingen i atomisk delgrafer}
...
\textit{dataflow graphs} that express the overall flow of data in the
program. The basic task in writing a Flask program is thus to connect
(or \textit{wirering}) incoming streams of sensor data to outgoing
streams of data (e.g. to other nodes in the network) with some
eventual data processing in between. A program thus takes form of an
acyclic graph.
...
Later we will examine how this is written in Flask.
...
node-level code is written in one of several object-languages (where
Haskell is the meta-language) for manipulating node-level values.
...
Mainland. This allows you to write the object languages in their
original syntax inside the Flask/Haskell environment. And it allows
you to intersperse generation time variables (which results in compile
time constants) into object-language code.

Figure \ref{fig:mainloop} illustrates this, showing a simplified
version of the main loop and initialization code from Fladuino.
\texttt{cdefs\_toc} and \texttt{cstms\_toc} are compile--time values
(lists of C statements), which are inserted into the syntax tree of
the parsed C-code at generation--time.
...
from Haskell, but is restricted to a very small subset of what is
...
huge to fit on the limited memory of a sensor node. Therefore we can't
...
which increments a value at every iteration until it surpasses another
input value. This is taken from the our example of a binary counter.
\begin{figure}
\begin{verbatim}
[$exp|\(x, s) -> if s > x then (0, 0) 
                          else (s+1, s+1)|]
\end{verbatim}
...
Writing dataflow graphs in Haskell is normally done by using the
Arrows library. But this isn't a strategy that fits Flask, because
...
therefore aren't Haskell functions, but instead a representation of of
...
In Flask a stream of values with type $\alpha$ has the type $S\
\alpha$. The arguably simplest way to generate such a stream is the
\texttt{clock :: Int -> S ()} Flask-function, which given a time
interval in milliseconds generates an empty signal on the resulting
stream. \texttt{clock 1000} is signalling every second. This isn't
very useful by itself, some data processing needs to occur, and for
this Flask provides a set of stream operators, as mentioned before.
...
Now to one of the Flask stream operators, \texttt{sfilter}. It's type
signature is:
...
signal functions that Flask provides.
\begin{description}
\item[\texttt{sconst}]\hfill \\ 
...
\item[\texttt{\&\&\&} and \texttt{szip}]\hfill \\ 
  
  
\item[\texttt{smap}] \hfill \\ Maps a node-level function over all
  stream-values, creating a stream of the resulting output values.
\item[\texttt{sintegrate}] \hfill \\ When supplied with an initial
  (node-level) state value and a node-level function that takes the
  current state and the next value on the input stream, returning a
  new state and the next value to be send through to the output
  stream.
\item[\texttt{clock :: Int -> S ()}] Creates a stream of unit-values
  that is fired with the given interval (specified in milliseconds).
\item[\texttt{sfilter}]\hfill \\ Only propagates values through to the
  output-stream that satisfies a given node-level predicate.
%\item[\texttt{sunzip}]\hfill \\ 
%\item[\texttt{smerge}]\hfill \\ 
...
Arduino devices in a functional reactive manner.  We have set the
following requirements for our final program: \fixme{eventuelt
  reference til synopsisen}
...
And we set the following limitations of our project scope:
\begin{itemize}
\item We shall not describe the implementation of any specific
  ...
  useful for evaluating Fladuino as a whole.  \fixme{jeg tror ikke
    dette behøves}
...
\item We won't add primitives for writing real time systems.
\end{itemize}
...
\textit{Flows} for communicating between devices) that build heavily
upon the TinyOS--platform for sensor networks, a platform that is not
available to us.  We have thus not adapted all of these facilities in
our work, though posting wires (see \Fref{sec:posting wires}) have
their own novel implementation in Fladuino.
...
\begin{figure}
  ...
  \includegraphics[width=0.9\textwidth]{images/accumulate-repeat-1}
  \includegraphics[width=0.9\textwidth]{images/accumulate-repeat-2}
  \includegraphics[width=0.9\textwidth]{images/accumulate-repeat-3}
...
\begin{figure}
  \label{fig:queue}
  \includegraphics[width=0.9\textwidth]{images/queue-1}
  \includegraphics[width=0.9\textwidth]{images/queue-2}
  \includegraphics[width=0.9\textwidth]{images/queue-3}
...
abstraction on top of events, with a unique event value associated
with each posting wire.  Implementing posting wires as a separate
subsystem offers no compelling advantages over an implementation via
abstractions built on the existing event--dispatch infrastructure.
...
\subsection{Defining a servo motor}


Stupid example. Rather use the 3pi instead.
