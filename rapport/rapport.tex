...
make any sense to define any events upon this\footnote{Events currently only
  ...
  the Atmega chip.} so its value must to be pulled once in a while to know if it
has changed.
...
explicitly changed. The scale 0 to 1023 when reading analog values is
...
exactly the same pin configuration as the \texttt{arduinoDuemilanove} platform,
