...
often\fixme{citations needed} defined as \textit{programmable systems that
...
A discrete \textit{node-level value} of type $a$ has the type $N a$ in
Haskell.  For example, node-level integers are referred to as $N
Integer$, and a stream of such integers would have the type $S
Integer$.
...
for sinks\fixme{sinks ikke forklaret tidligere} to manually signal
...
directly to this datatype, but we've instead done this in a more or
less enclosing type class, which is shown in \fref{fig:Event type
  class}. This type class serves the purpose of moving work from the
event-user, to the event-implementer. Instead of making the user
...
In the type class definition, the phantom type $t$ is the payload-type
and $e$ is a user-defined event-type (useful for saving the
\textit{Device} attached to the event). Notice, that we make use of
the GHC extension \textit{FunctionalDependencies} to specify that an
event can't have more than one type of payload (i.e. when we know the
event type, we also know the type of its payload).

When specifying a new event-type, most of the work to do is in the
\verb|setupEvent| function, which both initialises the event and
creates the associated \verb|ERep| value. To simplify the tedious task
