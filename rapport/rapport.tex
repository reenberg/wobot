...
\begin{abstract}
  ...
  language for programming embedded processors, which are scarce on
  resources, using a staged compilation-strategy like the one found in
  Flask \cite{flask08}, where code native to the hardware-platform is
  generated from a dataflow graph, which then is compiled to
  machine-code by a platform-specific compiler.

  In the creation we've used the Flask-codebase as our starting point
  and huge (isolated) parts of the code was left unchanged. Our
...
\fixme{dataflow eller data flow? --- ensret}
...
In this report we present our framework for programming embedded
devices, \textit{Fladuino}. Fladuino is an adaption of the sensor
network framework \textit{Flask} for the domain of control systems.
...
Our domain, control systems, is quite wide. We define it as
\textit{any programmable electronic system with sensors and/or
  actuators}\fixme{reformulation?}. This has the obvious impact, that
our framework should be general enough to support almost any thinkable
peripheral. We do this by making the user capable of writing drivers
or small frameworks for the different devices that users could connect
to the Arduino-boards.

\fixme{I find the following Reactivity-section relevant, but I don't know where
  it fits in. I don't think it's here /Martin}
\section{Reactivity}
...
connecting streams of values (a kind of ever-changing input values,
...
\begin{description}
\item because of its low cost. An Arduino Duemilanove is priced at approximately 200
  DKK at the time of writing.
\item because of its extensibility. You can buy plenty of different
  ...
  bluetooth, ethernet or even small touch, to name a few. It is also
  our perception that it isn't hard to connect it with other
...
\section{Motivation (very robots specific, should be rewritten for
  control systems)}
\textit{Something about how our everyday lifes are packed with control
systems. Dishwashers, etc. --- humanistic blah blah --- the following
is probably too robot-specific} 
...
In the following chapter (chapter \ref{chapter:Flask}) we will go
through the existing Flask system. Its design and how you use it to
write sensor-network programs.

In chapter \ref{chapter:design} we will look at what functionality is
...
In chapter \ref{chapter:evaluation} we will present an evaluation of
the complete Fladuino system, judging its benefits over traditional
methods of programming control programs, as well as covering the
...
Flask. We see that this is a single connected graph\footnote{This is
  ...
  section on posting wires, \ref{sec:posting wires}.}, but many Flask
...
before we can go on to the next action. \fixme{eventually add
  something about the need for this} This point is actually easier
to explain when talking about Fladuino. The good example here is that
we might want two different \textit{atomic subgraphs} to execute
depending on whether a button on our circuit was pushed or released.
...
in the Flask-implementation. The other object-language, Red, is described
in the next section (\ref{sec:red}).
...
threshold. The threshold is a generation-time computed value, which is
inserted at generation-time by the \texttt{flo}-antiquoter.
\begin{nonfloatingfigure}
\begin{verbatim}
...
classes: \textit{Reify} and \textit{LiftN}.  Instances of the Reify
type classes are Haskell types that has node-level equivalents. An
instance has to provide a function reify that given a Haskell value
returns a representation of its node-level type.
...
\item \texttt{($\ggg$) :: forall a. Reify a => S a -> (S a -> S a)}
  ...
  simply implemented as a normal Haskell function application.
...
\end{description}
\newpage
\section{The EWMA example in Flask-code}
The dataflow graph from figure \ref{ewma-example} can now be written
...
\item Fladuino must be able to generate a valid C-program suitable for
  ...
  \textit{This requirement stems from the selection of
...
Blocking operations are generally disallowed in the dataflow graph,
such an operation would clog the system, since there is no way to
continue down another path in the meantime. This was the basis for
adding posting wires (section \ref{sec:posting wires}). This gives us
a problem when it comes to pulling analog data from hardware sensors,
these functions requires some busy-waiting for the analog $-->$
...
\end{verbatim}
Here the functions waits for the analog $-->$ digital conversion to
finish, which is signaled by the clearing of a special register.
We can find a similar problem in the library that reads the Pololu
3pi's reflectance sensors.

Testing of these functions have shown that they generally tend to be
fast, blocking the processor for unnoticeable timeperiods. So our
solutions to this problem is simply to change the disallowance of
\textit{all} blocking functions to only disallowing calculations
that occupies the processor for several milliseconds.\fixme{Troels: er
  det hvad du mente?}
...
Most device-drivers also needs some additional data, e.g. about
which hardware-pin the device is connected to or the starting state of
the device (e.g. turned off).
...
dependencies etc. A simple solution would be to implement the all
devices as a general \textit{Device} datatype with the above values as
fields. But as discussed above devices differ and the additional data
required would be clumsy to store in such a general datatype.  A
second reason for why this isn't a good idea, is that we wouldn't have
a way of distinguishing devices on their types. We solve this by
requiring that the driver-writer creates a separate datatype for each
device and makes it an instance of our \textit{Device} typeclass. We
can thus identify devices by being instances of this typeclass and
specific devices can be identified directly by their datatype. The
program-state is extended with a list of these devices. The required
values are shown in figure \ref{device typeclass}.
...
parameter. This is because we can connect simultaneous with more than
one device of each device-type, and they differ only in the values
stored in the datatype-fields. These functions needs to know
specifically which instance of the device-type that is referred to.
...
\begin{description}
\item[Pins] This is a list of pin declarations. A pin declaration is either a
  digital pin or analog pin followed by a list of features supported by this pin
  and its pin number. A feature could be that pin supports ``PWM'' and ``interrupt''. 

\item[Pin map] This is a function that maps digital pin number or analog pin
  number to the physical platforms specific pin numbers. E.g. the Arduino
  Duemilanove maps analog pin 0 to physical pin 14.
\fixme{troels: hvad bruges dette egentlig til?}

\item[Capabilities] This is a list of platform specific capabilities. All
  platforms should include their name such that a Fladuino program can state
  only to be compatible with this platform. Thus all platforms that is
  compatible with each other could include this in the list of
  capabilities. Other capabilities could be the Pololu 3pi robot's motors,
  buzzer or LCD.

\item[Base setup] Any platform specific things that should be added like
  initialisation code for the Arduino BT platforms bluetooth device or inclusion
  of specific header file on the Pololu 3pi platform.

\item[Default devices] This is a list of default devices that a platform has
  mounted. E.g. motors, potentiometers and sensors on the Pololu 3pi robot.

\fixme{Er dette korrekt?}

\item[Timer id] This defines the timer id that Fladuino uses to drive the
  clock. Some Arduino compliant hardware reserves different timers for various
  tasks, like the Pololu 3pi uses time 0 and timer 1 to drive the motors. 
\end{description}
...
This is also the case when defining the \texttt{pololu3pi} platform, as it uses
the same Atmega processor and thus has the same pin configuration. It however
comes with a bunch of default devices and capabilities

\begin{verbatim}
aasd
\end{verbatim}

3 pi pin mapping \cite{3pi_pin_mapping}.
...
\begin{quotation}
  ...
  \begin{nonfloatingfigure}
    ...
    \caption{How 0\%, 25\%, 50\%, 75\% and 100\% PWM is measured by an
      oscilloscope. Each vertical line indicates the duty
      cycle}
    \label{fig:pwm}
  \end{nonfloatingfigure}
  
  \fixme{Better figure text}
