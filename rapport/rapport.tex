...
We expect that readers of this report can read Haskell, and functional
programming experience, as e.g. acquired from the book ``Real World
Haskell'' (\cite{realworldhaskell}).  Additionally, we expect the
reader to have casual familiarity with the concepts of Quasiquotation
...
these values (see \fref{sec:dataflowtranslation} for the implementation
...
facility, termed \textit{idle waiters} (see \Fref{sec:idle waiters}),
...
greatest common divisor is calculated (as argued in \fref{sec:runtime-system}) and used to calculate the timeout vale of
a hardware timer (see \fref{sec:timer-interrupts}). Each time an overflow
interrupt from the timer occurs a IHF (Interrupt Handler Function) is called
which takes care of queueing events in the event queue. This is done by having a
counter for each \texttt{clock} operator and knowing how many times the timer
must overflow before the defined \texttt{clock} time has elapsed. This handler
function is statically generated at compile-time. Currently timer 1 and 2 can be
used as the \texttt{clock}-timer since the Arduino framework reserves timer
0\footnote{This timer is used for the functions \texttt{millis()},
  \texttt{delay()} and other timing related function}.
...
data, e.g.  which hardware-pin the device is connected to or the
starting state of the device (e.g. turned off).
...
device initialization-phase of the code generation. The \verb|Device|
...
\section{A potentiometer device}
...
C-wrappers\footnote{Found in the ``6.a Downloading and Installing the
  C/C++ Library'' section of the Pololu website: \url{http://www.pololu.com/docs/0J21/6.a}}. Thus, as with the
...
calibration.  This is done in the C-code and not as Fladuino dataflow
graph, as we have no way of entering setup-code as a dataflow
graph. We will discuss this further in the our evaluation of
Fladuino, see \Fref{chap:evaluation}. This waiting is done so the user has a chance of placing the
robot correctly on the black line before calibration.
...
quite like the potentiometer example (\fref[plain]{sec:potentiometer
  device}), and we wont discuss it further.
...
input stream is two values, a rotational velocity and a translational
...
\fref{fig:set_motors implementation}.
...
compute (v, omega) = let motorLvel = v * (if omega <= 0.0 then 1.0 else 1.0 - omega)
                     ...
                     in (round (motorLvel * 255.0), round (motorRvel * 255.0))
                       |]
\end{verbatim}  
  \caption{Implementation af \texttt{set\_motors}}
...
network.  Flask thus has very crude support for internal state, in
particular a complete lack of support for dynamically allocated state,
which has the implication that the size of state data has to be fixed
at compile-time.  While this may not be a significant problem for the
programming of sensor networks, it can be a significant obstacle to
the programming of general reactive systems.  This, more than anything
else, is perhaps the chief deficiency that came about due to our
choice to adapt Flask, rather than writing a system from scratch.
...
In this section we will evaluate Fladuino based on another example: a
line following program for the Pololu 3pi.
...
As we explained in \Fref{sec:3pi bindings}, calibration code for the
sensors is automatically inserted into the initialisation of the
generated Arduino-code when the \verb|ReflectanceSensors|-device is
used. Also mentioned there, is the fact that we had to do write the
calibration code in C.  Thus implementing this example revealed an
improvement point for our system, as we could provide a way for
entering one or more atomic subgraphs that should be evaluated at
start-up. 
...
functionality stripped) is shown in \Fref{fig:linefollow-in-c}. The
used calibration function, described in \Fref{sec:3pi bindings} is
replicated here. The function \verb|set_motors_rot_trans| is a
conceptual function of ours, that takes rotational and translational
velocities like our \verb|set_motors|-function. We have introduced it
...
a simple junction of the set of dataflow graphs, as shown in the
following line of code.

\begin{verbatim}
main = defaultMain . genStreams . map unS $ [linefollow] ++ alarm
\end{verbatim}
...
\item Generalising and extending the platform abstraction, the runtime
  ...
  than Arduino.  Many microcontrollers are very similar to Arduino, in
  that they offer little more than a selection of I/O pins, and it
  should be relatively simple to modify Fladuino to support these as
  well.
\end{itemize}
