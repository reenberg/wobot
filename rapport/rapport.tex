...
And only a small subset of the Standard Prelude is included
in Flask. Type classes should be possible to implement though, since
it's would be done at compile-time, and thus shouldn't be compiled to C.
...
\begin{description}
\item[\texttt{sconst :: forall a b eta. (Reify b, LiftN eta a)
       => eta -> S b -> S a}]\hfill \\ 
     Maps all input value to a constant output value.
...
\item[\texttt{\&\&\&} (and its alias \texttt{szip})]\hfill \\
  This operator has two input streams. It waits for a value on both
  inputs, constructs a pair of them, and sends this pair two its
  output.

\item[\texttt{sunzip}]\hfill \\ 
  The inverse of \texttt{\&\&\&} and \texttt{szip}, which splits a
  signal of pairs into two individual output streams.
