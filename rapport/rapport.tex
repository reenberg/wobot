...
often\fixme{citations needed} defined as \textit{programmable systems
...
Node-level code of type $a$ has the type $N a$ in Haskell.  For
example, a node-level expression evaluating to an integer is referred
to as $N Integer$, and a stream of such integers would have the type
$S Integer$.
...
as a general \textit{Device} datatype with the above values as
...
second argument against this idea is that we wouldn't have a way of
distinguishing devices on their types. We solve this by requiring that
the driver-writer creates a separate datatype for each device and
makes it an instance of our \textit{Device} typeclass. We can thus
identify devices by being instances of this typeclass and specific
devices can be identified directly by their datatype. The
...
specify that an event can't have more than one type of payload
(i.e. when we know the event type, we also know the type of its
payload).
...
The \texttt{Device} instance definition \texttt{BatteryReader} requires that the
