...
can be lifted to a node-level \textit{Integer} (by the \texttt{liftN} function).\\
...
Most device-drivers also needs some additional data, e.g.  which
hardware-pin the device is connected to or the starting state of the
device (e.g. turned off).
...
dependencies etc. A simple solution would be to implement all
