...
Writing dataflow graphs in Haskell is something solved by the Arrows
library. But this isn't a strategy that fits in with Flask, because
arrows requires that an arbitrary Haskell function can be lifted to a
signal function. This isn't possible here, because signal functions
should run on the sensor nodes and therefore aren't real Haskell
functions, but instead some Haskell representation of the node-level
code that should be generated. Flask takes a little simpler approach,
and defines some of the same operators as in the arrows library,
though with different type signatures.
...
\texttt{clock :: Int -> S ()} Flask-function, which given an interval
in milliseconds generates a unit value on the resulting
stream. \texttt{clock 1000} is signalling every second.  This isn't
very useful by itself, ... something about node level values ($N\
\alpha$) etc.


\subsection{Streamoperators}
A description of the Flask stream-operators follows. 
\begin{description}
\item[\texttt{clock}] 
\item[$\ggg$]\hfill \\ Connects a stream with a stream
  combinator.
\item[\texttt{\&\&\&} and \texttt{szip}]\hfill \\ 
  
\item[\texttt{sconst}]\hfill \\ 
\item[\texttt{smap}] \hfill \\ Maps a node-level function over all stream-values,
  creating a stream of the resulting output values.
\item[\texttt{sintegrate}] \hfill \\ When supplied with an initial node-level state value
  and a node-level function that takes the current state and the next
  value on the input stream, returning a new state and the next value
  to be send through to the output stream.
\item[\texttt{sfilter}]\hfill \\ Only propagates values through to the output-stream that
  satisfies a given node-level predicate.
